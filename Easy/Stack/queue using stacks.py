...
class MyQueue:
    ...
    def push(self, x: int) -> None:
        # while there is stuff in first stack, empty it into the second stack
        while self.s1:
            self.s2.append(self.s1.pop())

        # now add the item you want to add to the first stack
        # this may be the first item if stack 1 is empty
        self.s1.append(x)

        # now take the second stack and empty it into the first stack
        while self.s2:
